...
from search.templatetags.django_slimmer import _static_file
import settings 
...
class Test__django_slimmer(TestCase):
    ...
    def test__static_file__debug_on_save_prefixed(self):
        """ test the private method _static_file().
        ...
        result_filename = _static_file(TEST_JS_FILENAME)
        ...
        assert result_filename == _static_file(TEST_JS_FILENAME)
    ...
    def test__static_file__debug_on_save_prefixed_name_prefixed(self):
        """ 
    ...
    def test__static_file__debug_on_save_prefixed_name_prefixed_image(self):
        ...
        result_filename = _static_file(TEST_GIF_FILENAME)
        assert result_filename != TEST_GIF_FILENAME, "It hasn't changed"
        ...
        # run it again to test that the _static_file() function can use
        # it's internal global variable map to get the file out
        assert result_filename == _static_file(TEST_GIF_FILENAME)
        ...
        time.sleep(1) # slow but necessary
        # now change the original file
        open(TEST_MEDIA_ROOT + TEST_GIF_FILENAME, 'w').write('var b  =  foo\n')
    ...
    def test__static_file__debug_off(self):
        """ same test as test__static_file__debug_on() but this time not
        ...
        result_filename = _static_file(TEST_CSS_FILENAME)
